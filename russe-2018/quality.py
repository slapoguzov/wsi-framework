...
def calculate_quality(dataset_fpath):
    df = read_csv(dataset_fpath, sep='\t', dtype={'gold_sense_id': str, 'predict_sense_id': str})
    ari, words = ari_per_word_weighted(df)
    print('{:<12}\t{:<12}\t{:<12}'.format('word', 'ari', 'count'))

    for word in sorted(words.keys()):
        print('{:<12}\t{:<12.6f}\t{:d}'.format(word, *words[word]))

    print('{:<12}\t{:<12.6f}\t{:d}'.format("amount", ari, len(df)))
